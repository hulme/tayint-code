{getSectors[name_,i_]:=Block[{FullName,functiontooptimize,SimplifiedSectorList,q},
FullName=(ToString[name]<>ToString[i]<>".m");
Import[ToString[FullName]];
SimplifiedSectorList=Rationalize[Simplify[functiontooptimize,TimeConstraint->3]];
Return[SimplifiedSectorList]]}
{PlainTay[func_,varlist_?VectorQ,conflist_?VectorQ,intstartlist_?VectorQ,intstoplist_?VectorQ,start_,ord_,name_,num_]:=Block[{Jac,confFunc,Num,ExpFunc,TaylorFunc,IntFunc,TaylorResult,alpha,sum,powlist,powvarlist,intlimlist,e,il,ih,pow,sumindexlist,sumlimlist,k,derlist,intminlist,intmaxlist,varlist2,conflistInv,y,explist},
...
{PlainTayKine[varlist_?VectorQ,knamelist_?VectorQ,kvallist_?VectorQ,namein_,nameout_,start_,ord_,secstart_,secnum_]:=Block[{sum,ResultListPre,ResultList,alpha,intminlist,intmaxlist,sumindexlist,sumlimlist,k},
{sumindexlist=k[#]&/@Range[1,Length[varlist]];}
{sumlimlist=({sumindexlist[[#]],0,ord})&/@Range[1,Length[varlist]];}
{sum=(Flatten[Table[ReplaceRepeated[alpha->Sequence[]][If[start<=(Plus@@sumindexlist)<=ord,sumindexlist,alpha]],Evaluate[Sequence@@sumlimlist]],Length[varlist]-1]);}
{Put[Sum[((Uncompress[(Import[(ToString[i]<>"TAYINTCO"<>ToString[namein]<>"["<>ToString[sum[[#]]]<>"].h")])])/.Thread[knamelist->kvallist]),{i,secstart,secnum}],("KINETAYINTCO"<>ToString[nameout]<>"["<>ToString[sum[[#]]]<>"].h")]&/@Range[1,Length[sum]];}
